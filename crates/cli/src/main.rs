...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        None => tui::run(cli.config.as_deref(), cli.profile.as_deref())?,
        ...
        Some(Commands::Reindex(args)) => cmd::reindex::run(
            cli.config.as_deref(),
            cli.profile.as_deref(),
            args.verbose,
            args.force,
        ...
        Some(Commands::Task(subcmd)) => match subcmd {
            TaskCommands::List(args) => cmd::task::list(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                args.project.as_deref(),
                args.status,
            )?,
            TaskCommands::Done(args) => cmd::task::done(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.task,
                args.summary.as_deref(),
            )?,
            TaskCommands::Cancel(args) => cmd::task::cancel(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.task,
                args.reason.as_deref(),
            )?,
            TaskCommands::Status(args) => cmd::task::status(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.task_id,
            )?,
        ...
        Some(Commands::Project(subcmd)) => match subcmd {
            ProjectCommands::List(args) => cmd::project::list(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                args.status,
                args.kind,
            )?,
            ProjectCommands::Status(args) => cmd::project::status(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.project,
            )?,
            ProjectCommands::Progress(args) => cmd::project::progress(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                args.project.as_deref(),
                args.json,
            ...
            ProjectCommands::Archive(args) => cmd::project::archive(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.project,
                args.yes,
        ...
        Some(Commands::Area(subcmd)) => match subcmd {
            AreaCommands::Report(args) => cmd::area::report(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.area,
                &args.period,
            ...
            AreaCommands::Export(args) => cmd::area::export(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.area,
                args.from.as_deref(),
        ...
        Some(Commands::Context(subcmd)) => match subcmd {
            ContextCommands::Day(args) => cmd::context::day(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                args.date.as_deref(),
                &args.format,
            ...
            ContextCommands::Week(args) => cmd::context::week(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                args.week.as_deref(),
                &args.format,
            )?,
            ContextCommands::Note(args) => cmd::context::note(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.path,
                &args.format,
            ...
            ContextCommands::Focus(args) => cmd::context::focus(
                cli.config.as_deref(),
                cli.profile.as_deref(),
                &args.format,
                args.with_tasks,
        ...
        Some(Commands::Dashboard(args)) => tui::dashboard::run(
            cli.config.as_deref(),
            cli.profile.as_deref(),
            args.project.as_deref(),
            args.activity_days,
