//! Palette list rendering (templates and captures).
...
pub fn draw(frame: &mut Frame, area: Rect, app: &App) {
    ...
    // Captures section
    if app.captures_start_index < app.items.len() {
        ...
        for (i, item) in app.items.iter().enumerate().skip(app.captures_start_index) {
            let style = if i == app.selected {
                Style::default().bg(Color::DarkGray).fg(Color::White)
            } else {
                Style::default()
