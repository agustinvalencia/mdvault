...
use charts_rs::{
    svg_to_png, BarChart, ChildChart, LineChart, MultiChart, PieChart, Series, THEME_GRAFANA,
};
...
pub fn generate_project_dashboard_png(
    ...
    let project = report
        .projects
        .first()
        .ok_or("No project data in report")?;
...
fn build_task_status_pie(report: &DashboardReport) -> PieChart {
    ...
    let series: Vec<Series> = [
        ...
        (
            "In Progress",
            status.get("in_progress").copied().unwrap_or(0),
        ),
        ("Blocked", status.get("blocked").copied().unwrap_or(0)),
        (
            "Cancelled",
            status.get("cancelled").copied().unwrap_or(0),
        ),
    ]
    .into_iter()
...
fn build_project_progress_bar(report: &DashboardReport) -> BarChart {
    ...
    let mut projects: Vec<&ProjectReport> = report
        .projects
        .iter()
...
fn build_velocity_bar(report: &DashboardReport) -> charts_rs::HorizontalBarChart {
    let mut projects: Vec<&ProjectReport> = report
        .projects
        .iter()
        .filter(|p| p.velocity.tasks_per_week_4w > 0.0)
        .collect();
    projects.sort_by(|a, b| {
        b.velocity
            .tasks_per_week_4w
            .partial_cmp(&a.velocity.tasks_per_week_4w)
            .unwrap()
    });

    let labels: Vec<String> = projects.iter().map(|p| p.id.clone()).collect();

    let vel_4w: Vec<f32> = projects
        .iter()
        .map(|p| p.velocity.tasks_per_week_4w as f32)
        .collect();
    let vel_2w: Vec<f32> = projects
        .iter()
        .map(|p| p.velocity.tasks_per_week_2w as f32)
        .collect();

    let series = vec![
        ("4-week avg", vel_4w).into(),
        ("2-week avg", vel_2w).into(),
    ];

    let mut bar = charts_rs::HorizontalBarChart::new_with_theme(series, labels, THEME_GRAFANA);
